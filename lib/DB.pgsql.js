...
DBdriver.prototype.getDBParam = function (dbtype, val) {
  ...
  if ((dbtype.name == 'VarChar') || (dbtype.name == 'Char')) {
    ...
    return "'" + escape(val.toString()) + "'::numeric("+dbtype.prec_h+","+dbtype.prec_l+")";
  }
  else if (dbtype.name == 'Float') {
    var valfloat = parseFloat(val);
    if (isNaN(valfloat)) { return "NULL"; }
    //return valfloat.toString();
    return "'" + escape(val.toString()) + "'::float("+dbtype.prec+")";
  }
...
function getContextSQL(context) {
  var rslt = "set sessionvars.appuser to '" + escape(context) + "';";
