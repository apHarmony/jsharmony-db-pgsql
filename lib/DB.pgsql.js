...
var pgsqlQueryStream = require('pg-query-stream');
...
DBdriver.prototype.getCapabilities = function(){
  return ['recordset','multirecordset','row','command','scalar','recordset_stream'];
};
...
DBdriver.prototype.getPooledConnection = function (dbconfig) {
  ...
  if(!pgpool.con){
    pgpool.con = new pgPool(_.extend({}, dbconfig, { options: undefined }));
...
DBdriver.prototype.getDBParam = function (dbtype, val) {
  ...
  if ((dbtype.name == 'VarChar') || (dbtype.name == 'Char')) {
    ...
    if ((dbtype.length == types.MAX) || (dbtype.length == -1)) return "'" + _this.escape(valstr) + "'::text";
...
DBdriver.prototype.ExecSession = function (dbtrans, dbconfig, session) {
  ...
  if (dbtrans) {
    ...
    if(dbconfig.options.pooled){
      let con = _this.getPooledConnection(dbconfig);
      ...
      let con = new pgsql.Client(_.extend({}, dbconfig, { options: undefined }));
...
DBdriver.prototype.ExecError = function(err, callback, errprefix, sql) {
  ...
  if(_.isFunction(callback))  return callback(err, null);
  else throw err;
};
...
DBdriver.prototype.ExecQueryStream = function(pgclient, sql, conComplete, callback) {
  var _this = this;
  var queueSize = callback && callback.queueSize || 1;
  var onRow = callback && callback.onRow; // function(row, row_cb){ }
  var onComplete = callback && callback.onComplete; // function(err){ }
  var onDrained = callback && callback.onDrained; // function(err){ }
  var onNotice = callback && callback.onNotice; // function(notice){ }
  var onWarning = callback && callback.onWarning; // function(warning){ }
  var readError = null;
  var isReadDone = false;
  var notice_handler = function (msg) {
    if(readError) return;
    if(msg.severity=='WARNING'){ if(onWarning) onWarning(new DB.Message(DB.Message.WARNING, msg.message)); }
    else{ if(onNotice) onNotice(new DB.Message(DB.Message.NOTICE, msg.message)); }
  };
  pgclient.removeAllListeners('notice');
  pgclient.on('notice', notice_handler);
  _this.logRawSQL(sql);
  var query = new pgsqlQueryStream(sql);
  var dbStream = pgclient.query(query);
  var rowStream = null;
  ...
  dbStream.on('error', function(err){
    ...
    setTimeout(function(){
      ...
      _this.ExecError(err, callback, 'SQL Error: ', sql);
    }, 1);
  });
  rowStream = new stream.Writable({
    objectMode: true,
    highWaterMark: queueSize,
    write(row, encoding, callback){
      if(onRow){
        onRow(row, function(err){
          if(err){
            dbStream.emit('error', err);
          }
          callback();
        });
        return true;
  ...
  rowStream.on('finish', function(){
    if(onDrained) onDrained(readError);
  });
  dbStream.pipe(rowStream);
};
...
DBdriver.prototype.Exec = function (dbtrans, context, return_type, sql, ptypes, params, callback, dbconfig) {
  if(!dbconfig) throw new Error('dbconfig is required');
  if(!_.includes(['recordset','multirecordset','row','command','scalar','recordset_stream'], return_type)) throw new Error('Return type not supported: '+(return_type||'').toString());
  var _this = this;
  ...
  _this.ExecSession(dbtrans, dbconfig, function (err, pgclient, presql, conComplete) {
    ...
    //Execute sql
    if(return_type=='recordset_stream'){
      _this.ExecQueryStream(pgclient, execsql, conComplete, callback);
    }
    else {
...
DBdriver.prototype.ExecTransTasks = function (execTasks, callback, dbconfig) {
  ...
  _this.ExecSession(null, dbconfig, function (err, pgclient, presql, conComplete) {
    ...
    //Begin transaction
    _this.ExecQuery(pgclient, presql + "start transaction", function () { /* Do nothing */ }, callback, function () {
