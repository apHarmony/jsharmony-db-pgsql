...
exports.postModelForm = function (ent, model, fields, keylist, sql_extfields, sql_extvalues, param_datalocks, datalockqueries) {
  ...
  if (!('sqlupdate' in model)) {
    var tbl = model.table;
    sql = 'with xrslt as (update ' + tbl + ' set ' + _.map(_.filter(fields,function(field){ return (field.sqlupdate!==''); }), function (field) { if (field && field.sqlupdate) return field.name + '=' + parseSQL(ent, field.sqlupdate); return field.name + '=@' + (field.name); }).join(',');
...
exports.getSearchTerm = function (ent, field, pname, search_value, comparison) {
  ...
  switch (ftype) {
    ...
    case 'char': //.replace(/[%_]/g,"\\$&")
      if (comparison == '=') { sqlsearch = fsql + ' ilike @' + pname; }
      else if (comparison == '<>') { sqlsearch = fsql + ' not ilike @' + pname; }
      else if (comparison == 'notcontains') { search_value = '%' + search_value + '%'; sqlsearch = fsql + ' not ilike @' + pname; }
      else if (comparison == 'beginswith') { search_value = search_value + '%'; sqlsearch = fsql + ' ilike @' + pname; }
      else if (comparison == 'endswith') { search_value = '%' + search_value; sqlsearch = fsql + ' ilike @' + pname; }
      else if ((comparison == 'soundslike') && (field.sql_search_sound)) { sqlsearch = parseSQL(ent, field.sql_search_sound).replace('%%%FIELD%%%', '@' + pname); }
      else { search_value = '%' + search_value + '%'; sqlsearch = fsql + ' ilike @' + pname; }
