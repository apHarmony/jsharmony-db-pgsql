...
var DBObjectSQL = require('./DB.pgsql.sql.object.js');
...
DBsql.prototype.getModelMultisel = function (jsh, model, lovfield, allfields, sql_foreignkeyfields, datalockqueries, lov_datalockqueries, param_datalocks) {
  ...
  sql += ' ,coalesce(' + jsh.map.code_val + '::text,' + lovfield.name + '::text) ' + jsh.map.code_val + ',coalesce(coalesce(' + jsh.map.code_txt + '::text,' + jsh.map.code_val + '::text),' + lovfield.name + '::text) ' + jsh.map.code_txt;
  ...
  else if ('code' in lov) { lovsql = 'select ' + jsh.map.code_val + ',' + jsh.map.code_txt + ',' + jsh.map.code_seq + ' from ' + jsh.map.code + '_' + lov['code'] + ' where (' + jsh.map.code_end_date + ' is null or ' + jsh.map.code_end_date + '>current_date)'; }
  else if ('code_sys' in lov) { lovsql = 'select ' + jsh.map.code_val + ',' + jsh.map.code_txt + ',' + jsh.map.code_seq + ' from ' + jsh.map.code_sys + '_' + lov['code_sys'] + ' where (' + jsh.map.code_end_date + ' is null or ' + jsh.map.code_end_date + '>current_date)'; }
  else if ('code_app' in lov) { lovsql = 'select ' + jsh.map.code_val + ',' + jsh.map.code_txt + ',' + jsh.map.code_seq + ' from ' + jsh.map.code_app + '_' + lov['code_app'] + ' where (' + jsh.map.code_end_date + ' is null or ' + jsh.map.code_end_date + '>current_date)'; }
...
DBsql.prototype.putModelForm = function (jsh, model, fields, keys, sql_extfields, sql_extvalues, encryptedfields, hashfields, enc_datalockqueries, param_datalocks) {
  ...
  if (keys.length >= 1){
    if('sqlgetinsertkeys' in model) {
      sql += ' returning *) ' + model.sqlgetinsertkeys;
    } else {
      var sqlgetinsertkeys = ' returning ' + _.map(keys, function (field) { return field.name + ' as ' + field.name; }).join(',') + ') select ' + _.map(keys, function (field) { return field.name; }).join(',') + ' from xrslt;';
  ...
  if ((encryptedfields.length > 0) || !_.isEmpty(hashfields)) {
    var tbl = _this.getTable(jsh, model);
    enc_sql = 'with xrslt as (update ' + tbl + ' set ' + _.map(encryptedfields, function (field) { var rslt = field.name + '=' + XtoDB(jsh, field, '@' + field.name); return rslt; }).join(',');
...
DBsql.prototype.postModelForm = function (jsh, model, fields, keys, sql_extfields, sql_extvalues, hashfields, param_datalocks, datalockqueries) {
  ...
  var tbl = _this.getTable(jsh, model);
  sql = 'with xrslt as (update ' + tbl + ' set ' + _.map(_.filter(fields,function(field){ return (field.sqlupdate!==''); }), function (field) { if (field && field.sqlupdate) return field.name + '=' + _this.ParseSQL(field.sqlupdate); return field.name + '=' + XtoDB(jsh, field, '@' + field.name); }).join(',');
...
DBsql.prototype.postModelMultisel = function (jsh, model, lovfield, lovvals, foreignkeyfields, param_datalocks, datalockqueries, lov_datalockqueries) {
  ...
  var tbl = _this.getTable(jsh, model);
  sql = 'with xrslt_delete as (delete from ' + tbl + ' where (%%%SQLWHERE%%%) ';
  ...
  if (lovvals.length > 0) {
    ...
    sql += jsh.map.code_val + ' from (%%%LOVSQL%%%) multiparent where ' + jsh.map.code_val + '::text in ('
    for (var i = 0; i < lovvals.length; i++) { if (i > 0) sql += ','; sql += XtoDB(jsh, lovfield, '@multisel' + i); }
...
DBsql.prototype.deleteModelForm = function (jsh, model, keys, datalockqueries) {
  ...
  var tbl = _this.getTable(jsh, model);
  sql += 'with xrslt as (delete from ' + tbl + ' where (%%%SQLWHERE%%%) %%%DATALOCKS%%%';
  _.each(keys, function (field) { sql += ' and ' + field.name + '=' + XtoDB(jsh, field, '@' + field.name); });
  sql += ' returning 1) select count(*) xrowcount from xrslt;';
...
DBsql.prototype.getSearchTerm = function (jsh, model, field, pname, search_value, comparison) {
  ...
  switch (ftype) {
    ...
    case 'char': //.replace(/[%_]/g,"\\$&")
      if (comparison == '=') { sqlsearch = fsql + ' ilike ' + pname_param; }
      else if (comparison == '<>') { sqlsearch = fsql + ' not ilike ' + pname_param; }
      else if (comparison == 'notcontains') { search_value = '%' + search_value + '%'; sqlsearch = fsql + ' not ilike ' + pname_param; }
      else if (comparison == 'beginswith') { search_value = search_value + '%'; sqlsearch = fsql + ' ilike ' + pname_param; }
      else if (comparison == 'endswith') { search_value = '%' + search_value; sqlsearch = fsql + ' ilike ' + pname_param; }
      ...
      else { search_value = '%' + search_value + '%'; sqlsearch = fsql + ' ilike ' + pname_param; }
...
DBsql.prototype.getLOV = function (jsh, fname, lov, datalockqueries, param_datalocks, options) {
  ...
  else if ('code' in lov) { sql = 'select ' + jsh.map.code_val + ',' + jsh.map.code_txt + ' from '+(lov.schema?lov.schema+'.':'')+ jsh.map.code + '_' + lov['code'] + ' where (' + jsh.map.code_end_date + ' is null or ' + jsh.map.code_end_date + '>current_date) order by ' + jsh.map.code_seq + ',' + jsh.map.code_txt; }
  else if ('code2' in lov) { sql = 'select ' + jsh.map.code_val + '1 as ' + jsh.map.code_parent + ',' + jsh.map.code_val + '2 as ' + jsh.map.code_val + ',' + jsh.map.code_txt + ' from '+(lov.schema?lov.schema+'.':'')+ jsh.map.code2 + '_' + lov['code2'] + ' where (' + jsh.map.code_end_date + ' is null or ' + jsh.map.code_end_date + '>current_date) order by ' + jsh.map.code_seq + ',' + jsh.map.code_txt; }
  else if ('code_sys' in lov) { sql = 'select ' + jsh.map.code_val + ',' + jsh.map.code_txt + ' from '+(lov.schema?lov.schema+'.':'')+ jsh.map.code_sys + '_' + lov['code_sys'] + ' where (' + jsh.map.code_end_date + ' is null or ' + jsh.map.code_end_date + '>current_date) order by ' + jsh.map.code_seq + ',' + jsh.map.code_txt; }
  else if ('code2_sys' in lov) { sql = 'select ' + jsh.map.code_val + '1 as ' + jsh.map.code_parent + ',' + jsh.map.code_val + '2 as ' + jsh.map.code_val + ',' + jsh.map.code_txt + ' from '+(lov.schema?lov.schema+'.':'')+ jsh.map.code2_sys + '_' + lov['code2_sys'] + ' where (' + jsh.map.code_end_date + ' is null or ' + jsh.map.code_end_date + '>current_date) order by ' + jsh.map.code_seq + ',' + jsh.map.code_txt; }
  else if ('code_app' in lov) { sql = 'select ' + jsh.map.code_val + ',' + jsh.map.code_txt + ' from '+(lov.schema?lov.schema+'.':'')+ jsh.map.code_app + '_' + lov['code_app'] + ' where (' + jsh.map.code_end_date + ' is null or ' + jsh.map.code_end_date + '>current_date) order by ' + jsh.map.code_seq + ',' + jsh.map.code_txt; }
