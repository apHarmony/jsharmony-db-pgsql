...
DBsql.prototype.getModelMultisel = function (ent, model, lovfield, allfields, sql_foreignkeyfields, datalockqueries, lov_datalockqueries, param_datalocks) {
  ...
  sql += ') ' + tbl_alias;
  sql += ' full outer join (%%%LOVSQL%%%) multiparent on multiparent.' + ent.map.codeval + ' = ' + tbl_alias + '.' + lovfield.name;
...
DBsql.prototype.getLOV = function (ent, fname, lov, datalockqueries, param_datalocks, options) {
  ...
  else if ('UCOD' in lov) { sql = 'select ' + ent.map.codeval + ',' + ent.map.codetxt + ' from '+(lov.schema?lov.schema+'.':'')+'UCOD_' + lov['UCOD'] + ' where (CODETDT is null or CODETDT>current_date) order by ' + ent.map.codeseq + ',' + ent.map.codetxt; }
  else if ('UCOD2' in lov) { sql = 'select ' + ent.map.codeval + '1 as ' + ent.map.codeparent + ',' + ent.map.codeval + '2 as ' + ent.map.codeval + ',' + ent.map.codetxt + ' from '+(lov.schema?lov.schema+'.':'')+'UCOD2_' + lov['UCOD2'] + ' where (CODETDT is null or CODETDT>current_date) order by ' + ent.map.codeseq + ',' + ent.map.codetxt; }
  else if ('GCOD' in lov) { sql = 'select ' + ent.map.codeval + ',' + ent.map.codetxt + ' from '+(lov.schema?lov.schema+'.':'')+'GCOD_' + lov['GCOD'] + ' where (CODETDT is null or CODETDT>current_date) order by ' + ent.map.codeseq + ',' + ent.map.codetxt; }
  else if ('GCOD2' in lov) { sql = 'select ' + ent.map.codeval + '1 as ' + ent.map.codeparent + ',' + ent.map.codeval + '2 as ' + ent.map.codeval + ',' + ent.map.codetxt + ' from '+(lov.schema?lov.schema+'.':'')+'GCOD2_' + lov['GCOD2'] + ' where (CODETDT is null or CODETDT>current_date) order by ' + ent.map.codeseq + ',' + ent.map.codetxt; }
