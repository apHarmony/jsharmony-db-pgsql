...
declare '+tableName+' ALIAS FOR NEW; insert_id bigint;\n\
begin\n\
...
DBObjectSQL.prototype.restructureInit = function(jsh, module, obj){
  ...
  if(obj.type=='table'){
    ...
    sql += createTrigger(obj.name, 'before insert', triggers.validate_insert, '_1_validate_insert');
    sql += createTrigger(obj.name, 'before update', triggers.validate_update, '_1_validate_update');
    sql += createTrigger(obj.name, 'before insert', triggers.insert, '_2_insert');
    sql += createTrigger(obj.name, 'before update', triggers.update, '_2_update');
    sql += createTrigger(obj.name, 'before delete', triggers.delete, '_2_delete');
  }
  else if(obj.type=='view'){
...
DBObjectSQL.prototype.restructureDrop = function(jsh, module, obj){
  ...
  _.each(['validate_insert','validate_update','insert','update','delete'], function(op){
    if(triggers[op]){
      var eventName = '';
      if(obj.type=='table'){
        if(op=='validate_insert') eventName = "_1_validate_insert";
        else if(op=='validate_update') eventName = "_1_validate_update";
        else if(op=='insert') eventName = "_2_insert";
        else if(op=='update') eventName = "_2_update";
        else if(op=='delete') eventName = "_2_delete";
      }
      else if(obj.type=='view'){
        eventName = "_"+op;
      }
