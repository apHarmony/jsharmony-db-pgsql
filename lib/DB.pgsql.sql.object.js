...
DBObjectSQL.prototype.init = function(jsh, module, obj){
  ...
  if(obj.type=='table'){
    ...
    if(sqlprimarykeys.length) sqlcols.push('  primary key (' + sqlprimarykeys.join(',') + ')');
    sql += sqlcols.join(',\n') + '\n';
...
'  IF pg_trigger_depth() > 1 THEN\n\
    IF current_setting(\'sessionvars.last_trigger_source\') = \''+functionName+'\' THEN\n\
      --RAISE NOTICE \'recursion blocked % depth %\', TG_NAME,pg_trigger_depth();\n\
      return NULL;\n\
    END IF;\n\
  END IF;\n\
