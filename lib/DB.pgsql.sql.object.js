...
DBObjectSQL.prototype.init = function(jsh, module, obj){
  ...
  if('sql_create' in obj) sql = DB.util.ParseMultiLine(obj.sql_create)+'\n';
    ...
    if(!module.schema && obj.name){
      sql += 'grant select,insert,delete,update on table '+obj.name+' to {schema}_%%%INIT_DB_LCASE%%%_role_exec;\n';
    }
  }
...
function createTrigger(module, objName, triggerType, trigger, eventName) {
  var _this = this;
...
begin\n\
  set search_path = '+(module.schema||_this.db.getDefaultSchema())+',pg_catalog;\n';
...
'end;\n\
$trigger$;\n\
'+((!module.schema)?'grant all on function '+functionName+'() to {schema}_%%%INIT_DB_LCASE%%%_role_exec;\n':'')+'\
'+((!module.schema)?'grant all on function '+functionName+'() to {schema}_%%%INIT_DB_LCASE%%%_role_dev;\n':'')+'\
...
DBObjectSQL.prototype.restructureInit = function(jsh, module, obj, prefix){
  ...
  if(obj.type=='table'){
    ...
    sql += createTrigger(module, obj.name, 'before insert', triggers.validate_insert, '_'+prefix+'validate_insert');
    sql += createTrigger(module, obj.name, 'before update', triggers.validate_update, '_'+prefix+'validate_update');
    sql += createTrigger(module, obj.name, 'after insert', triggers.insert, '_'+prefix+'insert');
    sql += createTrigger(module, obj.name, 'after update', triggers.update, '_'+prefix+'update');
    sql += createTrigger(module, obj.name, 'after delete', triggers.delete, '_'+prefix+'delete');
  }
  else if(obj.type=='view'){
    sql += createTrigger(module, obj.name, 'instead of insert', triggers.insert, '_'+prefix+'insert');
    sql += createTrigger(module, obj.name, 'instead of update', triggers.update, '_'+prefix+'update');
    sql += createTrigger(module, obj.name, 'instead of delete', triggers.delete, '_'+prefix+'delete');
  }
  ...
  if (sql != '') sql = 'set sessionvars.last_trigger_source to \'\';\n' + sql;
  return sql;
};
