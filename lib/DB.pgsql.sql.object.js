  ...
  if('sql_create' in obj) sql = DB.util.ParseMultiLine(obj.sql_create);
    ...
    for(var i=0;i<sqlunique.length;i++){
      ...
      if(uniq && uniq.length){
        ...
        for(var j=1;(unqname in unique_names);j++) unqname = baseunqname.substr(0,62-(j.toString().length)) + '_' + j.toString();
    ...
    if(obj.index && obj.index.length){
      ...
      for(var i=0;i<obj.index.length;i++){
        ...
        if(index && index.columns && index.columns.length){
          ...
          for(var j=1;(idxname in index_names);j++) idxname = baseidxname.substr(0,62-(j.toString().length)) + '_' + j.toString();
...
DBObjectSQL.prototype.trimIdentifier = function(val){
  if(val && val.length > 64){
    var rslt = val.substr(0,48) + crypto.createHash('md5').update(val).digest("hex").substr(0,16);
  }
}
...
function createTrigger(objName, triggerType, trigger, eventName) {
...
DBObjectSQL.prototype.restructureInit = function(jsh, module, obj, prefix){
  ...
  if(obj.type=='table'){
    ...
    sql += createTrigger(obj.name, 'before insert', triggers.validate_insert, '_'+prefix+'validate_insert');
    sql += createTrigger(obj.name, 'before update', triggers.validate_update, '_'+prefix+'validate_update');
    sql += createTrigger(obj.name, 'after insert', triggers.insert, '_'+prefix+'insert');
    sql += createTrigger(obj.name, 'after update', triggers.update, '_'+prefix+'update');
    sql += createTrigger(obj.name, 'after delete', triggers.delete, '_'+prefix+'delete');
  }
  else if(obj.type=='view'){
    sql += createTrigger(obj.name, 'instead of insert', triggers.insert, '_'+prefix+'insert');
    sql += createTrigger(obj.name, 'instead of update', triggers.update, '_'+prefix+'update');
    sql += createTrigger(obj.name, 'instead of delete', triggers.delete, '_'+prefix+'delete');
  }
