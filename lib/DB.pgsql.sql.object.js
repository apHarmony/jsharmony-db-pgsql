...
DBObjectSQL.prototype.init = function(jsh, module, obj){
  var _this = this;
  var sql = 'set search_path = '+module.schema+',pg_catalog;\n';
  var caption = ['','',''];
  if(obj.caption){
  ...
  if('sql_create' in obj) sql = DB.util.ParseMultiLine(obj.sql_create)+'\n';
    ...
    if(sql.with){
      sql += ' with ';
      var first_with = true;
      for(var withName in sql.with){
        var withExpr = sql.with[withName];
        if(!first_with) sql += ',';
    ...
    for(var tblname in obj.tables){
      ...
      if(tbl.join_type){
        ...
        else if(tbl.table) join += tbl.table + ' as '+tblname;
...
begin\n\
  set search_path = '+module.schema+',pg_catalog;\n';
